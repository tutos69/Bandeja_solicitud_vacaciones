...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'startDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      startDate: employee.startDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      startDate: restEmployee.startDate ? dayjs(restEmployee.startDate) : undefined,
    };
  }
